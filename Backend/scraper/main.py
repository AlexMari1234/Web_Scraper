...
browser_url = f'wss://{auth}@{cred["proxy"]["default"]}'
...
async def search(metadata, page, search_text):
    try:
        ...
        if search_field_query and search_button_query:
            ...
            await search_box.type(search_text)
            print("Pressing search button")
            button = await page.wait_for_selector(search_button_query, timeout=60000)
            await button.click()
        else:
            raise Exception("Could not search.")
...
async def get_browser_instance(pw):
    for _ in range(MAX_RETRIES):
        try:
            browser = await pw.chromium.connect_over_cdp(browser_url)
            return browser
        except Exception as e:
            print(f"Error connecting to browser. Retrying... Error: {e}")
    raise Exception("Failed to connect to browser after multiple retries.")
...
async def main(url, search_text, response_route):
    ...
    async with async_playwright() as pw:
        browser = await get_browser_instance(pw)
        ...
        try:
            ...
            # Adding a delay after navigation to ensure all elements load
